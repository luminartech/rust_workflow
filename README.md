...
- **Comprehensive path.** Fuzz targets run for `fuzz-max-time` seconds normally
  ...
  manual dispatch can force it with `comprehensive-tests: true`. Set
  `miri-comprehensive-only: false` to run miri on pull requests too, which is
  worth it for crates where miri is fast.
